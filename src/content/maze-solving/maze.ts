...
class Maze<F extends Frontier> {
	...
	loadFromString(mazeString: string) {
		...
		this.solvable = true;
		this.explored = [];
		this.frontier.add(new Node(this.start, null, null, 0, this.heuristic(this.start, this.end)));
		// First step
	...
	move(): Node {
		...
		for (const neighbor of this.neighbors(node!.state)) {
			const child = new Node(neighbor, node, null, node!.g + 1, this.heuristic(neighbor, this.end));

			if (
				!this.frontier.containsState(child.state) &&
				!this.explored.some(
					(node) => node.state.x === child.state.x && node.state.y === child.state.y
				)
			) {
