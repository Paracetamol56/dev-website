<script lang="ts">
	...
	export const updateGraph = () => {
		if (!$maze.explored.length) return;
		root = d3
			.stratify()
...
<div class="w-full" bind:clientWidth={containerWidth}>
	<svg class="bg-ctp-mantle rounded-md shadow-md shadow-ctp-crust" {width} {height}>
		<g transform={`translate(${margin.left}, ${margin.top})`}>
            ...
            `}
				...
				{#each root.descendants() as node}
					<circle cx={node.y} cy={node.x} r="5" class="fill-ctp-mauve" />
					<text
						x={node.y}
						y={node.x + 14}
						dy="0.31em"
