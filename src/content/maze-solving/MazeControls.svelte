...
<div
	class="mb-8 p-4 rounded-md bg-ctp-mantle shadow-md shadow-ctp-crust flex flex-col items-start md:flex-row md:items-end justify-center gap-2"
>
	...
	<MeltSelect
		...
		value={algorithm}
	/>
	<Button on:click={() => $maze.aiStep()}>
		<span>Single step</span>
		<RedoDot size="18" />
	</Button>
	<Button on:click={() => $maze.aiSolve()}>
		<span>Solve</span>
		<Rocket size="18" />
	</Button>
</div>
