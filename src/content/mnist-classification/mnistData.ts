...
export class MnistData {
	...
	async load(nTrain = 40000, nTest = 10000) {
		...
		// Label request
		const labelRequest = axios
			.get(MNIST_LABELS_PATH, { responseType: 'arraybuffer' })
			.then((response) => {
				this.datasetLabels = new Uint8Array(response.data);
			})
			.catch((error) => {
				console.error(error);
			});

		await Promise.all([imageRequest, labelRequest]);
	 ...
	 */
	getTrainData() {
		const x_train = tf.tensor4d(this.trainImages!, [
			this.trainImages!.length / IMAGE_SIZE,
			IMAGE_H,
			IMAGE_W,
			1
		]);
		const y_train = tf.tensor2d(this.trainLabels!, [
			this.trainLabels!.length / N_CLASSES,
			N_CLASSES
		]);
	 ...
	 *     `[numTestExamples, 10]`.
	 */
	getTestData(numExamples?: number): [tf.Tensor4D, tf.Tensor2D] {
		let x_test = tf.tensor4d(this.testImages!, [
...
// modified again from https://github.com/adf-telkomuniv/TensorFlowJS_Tutorial/blob/master/part_3/mnist_data.js
// @author MATHEO GALUBA
// Earth - 2023
