<script lang="ts">
  ...
  const load = async (e: Event) => {
    ...
    try {
      $model = await tf.loadLayersModel(tf.io.browserFiles([jsonFile, weightsFile])) as tf.Sequential;
      addToast({
        data: {
          title: 'Model Loaded',
  ...
  <div class="w-full">
    ...
    <div class="flex flex-col items-start gap-2">
      <div class="flex bg-ctp-mantle rounded-md items-center gap-2 shadow-md shadow-ctp-crust">
        <label class="flex items-center gap-1 rounded-md bg-ctp-mauve px-3 py-1 font-semibold text-ctp-mantle
                      shadow-md shadow-ctp-crust transition-opacity hover:opacity-80 active:opacity-60" for="json-upload"
        >
          <span class="whitespace-nowrap">Browse JSON</span>
          <Braces size="18" stroke-width="3" />
        </label>
        <input type="file" id="json-upload" name="json-upload" class="hidden" bind:files={jsonFileList} />
      ...
      <div class="flex bg-ctp-mantle rounded-md items-center gap-2 shadow-md shadow-ctp-crust">
        <label class="flex items-center gap-1 rounded-md bg-ctp-mauve px-3 py-1 font-semibold text-ctp-mantle
                      shadow-md shadow-ctp-crust transition-opacity hover:opacity-80 active:opacity-60" for="weights-upload">
          <span class="whitespace-nowrap">Browse Weights</span>
          <Binary size="18" stroke-width="3" />
        </label>
        <input type="file" id="weights-upload" name="weights-upload" class="hidden" bind:files={weightsFileList} />
