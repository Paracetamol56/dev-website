...
<script lang="ts">
  ...
  $: colorLoss = d3.scaleSequentialSqrt(d3.interpolateReds)
    .domain([0, LossMax]);
  $: scaleLoss = Object.assign(
    colorLoss
      .copy()
      .interpolator(
        d3.interpolateRound(height - margin.bottom, margin.top)
      ),
      {range() { return [height - margin.bottom, margin.top] }}
  );
  $: colorTP = d3.scaleSequentialSqrt(d3.interpolateBlues)
    .domain([0, TPMax]);
