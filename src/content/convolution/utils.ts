...
async function applyConvolution(matrix: number[][], imageTensor: tf.Tensor4D): Promise<tf.Tensor4D> {
  if (!browser) {
    throw new Error('This code must be run in a browser environment');
  }
  ...
  const convolvedImageTensor = tf.tidy(() => {
    return tf.depthwiseConv2d(imageTensor, kernel, 1, 'same')
      .clipByValue(0, 1) as tf.Tensor4D;
  });
...
async function applyMaxPooling(poolSize: number, imageTensor: tf.Tensor4D): Promise<tf.Tensor4D> {
  if (!browser) {
    throw new Error('This code must be run in a browser environment');
  }

  const pooledImageTensor = tf.tidy(() => {
    return tf.maxPool(imageTensor, poolSize, poolSize, 'same') as tf.Tensor4D;
  });
