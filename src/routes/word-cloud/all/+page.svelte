        ...
        <div class="flex justify-start items-baseline mb-4">
          {#if session.open}
            <div class="relative square-2 mr-2 bg-ctp-green rounded-full">
              <span class="animate-ping absolute top-0.5 right-0.5 block square-1 rounded-full ring-2 ring-ctp-green bg-ctp-green"></span>
            </div>
            <p class="text-ctp-subtext0 text-sm">Open</p>
          {:else}
          <div class="relative square-2 mr-2 bg-ctp-red rounded-full"></div>
          ...
          <p class="ml-auto text-ctp-subtext0 text-sm">
            {session.submitions} submition{session.submitions <= 1 ? "" : "s"}
				...
				<a href="/word-cloud/{session.id}">
					<h4 class="mb-4 text-2xl font-bold hover:opacity-75 transition-opacity">{session.name}</h4>
				</a>
