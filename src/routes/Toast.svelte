...
<div
	...
	class="ctp-{theme} relative rounded-lg bg-ctp-surface2 text-ctp-text shadow-md"
>
	<div
		use:melt={$progress}
	...
	>
		<div
			class="h-full w-full bg-ctp-mauve"
	...
	>
		...
		<button
			use:melt={$close(id)}
			class="absolute right-4 top-4 grid place-items-center rounded-full text-ctp-text square-6
			hover:bg-ctp-mauve hover:text-ctp-surface2"
		>
			<X class="square-4" />
		</button>
	</div>
