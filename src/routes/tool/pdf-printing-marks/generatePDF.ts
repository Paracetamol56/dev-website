...
// PDF processing
export async function generatePDF(uploadedFile: File, options: PrintingMarksOptions): Promise<Blob> {
    ...
    fileReader.onload = async function () {
      ...
      for (let i = 0; i < pdfDoc.getPageCount(); i++) {
        ...
        if (options.cropMarks) {
          drawCropMarks(newPage, margin, options.unit, options.bleeds || { top: 0, right: 0, bottom: 0, left: 0 });
        }
...
async function drawPageInfo(newPdf: PDFDocument, margin: number, page: PDFPage, i: number, pdfDoc: PDFDocument, options: {
  pageNumbering: boolean;
  pageInfoText: string;
}) {
...
function drawCropMarks(page: PDFPage, margin: number, unit: "mm" | "in", bleeds: {
  top: number;
  right: number;
  bottom: number;
  left: number;
