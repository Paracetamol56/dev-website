<script lang="ts">
	...
	import { createSelect, melt, type CreateSelectProps, createRadioGroup, type CreateRadioGroupProps } from '@melt-ui/svelte';
...
<form class="grid grid-cols-1 gap-x-8 gap-y-6" on:submit={handleSave}>
	<fieldset class="max-w-xl">
		<label
			for="flavour"
			class="flex items-center gap-1 mb-2 text-sm font-semibold"
		>
			<Palette size="16" />
			<span>Flavour</span>
		</label>
		<div
			class="grid grid-cols-1 sm:grid-cols-2 rounded-lg bg-ctp-base gap-3"
			use:melt={$root}
		>
			{#each Object.keys(variants) as variant, i}
				<div
                    ...
                    {$isChecked(variant) ? 'ring-ctp-mauve text-ctp-mauve' : 'ring-ctp-crust text-ctp-text'}
				...
				>
					...
					<div class="pl-4 h-full">
						...
						<div class="h-4 w-2/3 mb-4 rounded-full bg-ctp-mantle"></div>
						<div class="h-6 w-1/3 mb-4 rounded-full bg-ctp-mauve"></div>
